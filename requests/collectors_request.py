...
directory_path = "outputs/collectors"
if os.path.exists(directory_path):
    shutil.rmtree(directory_path)

url = "https://hardwax.com/collectors-items.json"
...
while True:
    ...
    with open(f"{directory_path}/collectors_{page}.json", "w") as f:
...
for page in range(1, results_count // 32 + 2):
    with open(f"{directory_path}/collectors_{page}.json", "r") as f:
        all_results.extend(json.load(f)["response"]["results"])

with open(f"{directory_path}/collectors_all.json", "w") as f:
