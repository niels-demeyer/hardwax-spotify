...
directory_path = "outputs/colundi"
if os.path.exists(directory_path):
    shutil.rmtree(directory_path)

url = "https://hardwax.com/colundi-everyone.json"
...
while True:
    ...
    with open(f"{directory_path}/colundi_{page}.json", "w") as f:
        # Use the unescaped data when writing to the file
        json.dump({"response": {"results": unescaped_data}}, f)

    page_results_count = len(data["response"]["results"])
    results_count += page_results_count
    print(f"Page {page} contains {page_results_count} items.")

    try:
        if not data["response"]["next"]:
            break
    except KeyError:
...
all_results = []
for page in range(1, results_count // 32 + 1):
    with open(f"{directory_path}/colundi_{page}.json", "r") as f:
        all_results.extend(json.load(f)["response"]["results"])

with open(f"{directory_path}/colundi_all.json", "w") as f:
