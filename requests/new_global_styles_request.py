...
directory_path = "outputs/new_global_styles"
if os.path.exists(directory_path):
    shutil.rmtree(directory_path)

url = "https://hardwax.com/records.json"
...
while True:
    ...
    with open(f"{directory_path}/new_global_styles_{page}.json", "w") as f:
...
for page in range(1, results_count // 32 + 2):
    with open(f"{directory_path}/new_global_styles_{page}.json", "r") as f:
        all_results.extend(json.load(f)["response"]["results"])

with open(f"{directory_path}/new_global_styles_all.json", "w") as f:
