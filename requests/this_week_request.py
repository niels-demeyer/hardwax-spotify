...
directory_path = "outputs/this_week"
...
while True:
    ...
    with open(f"{directory_path}/this_week_{page}.json", "w") as f:
...
for page in range(1, results_count // 32 + 2):
    with open(f"{directory_path}/this_week_{page}.json", "r") as f:
        all_results.extend(json.load(f)["response"]["results"])

with open(f"{directory_path}/this_week_all.json", "w") as f:
